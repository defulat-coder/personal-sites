...
- 刊头承载全部内容入口（每日动态/每日关注/抖音收藏/开源关注/构建/问一问）：问一问虽是对话式动作，仍与阅读版块等权出现在版块序列末位；`/ask` 页的刊名是"问一问"，兄弟位与其余栏目页一致。
...
- 阅读版块各有独立路径（`/ai-news`、`/curation`、`/douyin`、`/open-source`、`/works`），刊头兄弟链接与移动导航都指向完整路径；首页遗留的 `?view=` 仅为旧链接兼容，不再是导航机制。
