...
- 刊头承载全部内容入口（每日动态/推特点赞/开源关注/构建/问一问）：问一问虽是对话式动作，仍与阅读版块等权出现在版块序列末位；`/ask` 页的刊名是"问一问"，兄弟位与其余栏目页一致。
...
- 每条内容是一行登记簿记录，不是卡片：左列是固定宽（`minmax(7rem,8.5rem)`）的 quiet 登记栏（日期、作者、附件逐行，`0.72rem`；附件是从 media/quoteContext 归并的登记词——图片/视频/GIF/引用，有才出现），右列是标题与解析；不使用方向箭头，整行即是链接。
- 剪报簿结构：解析之下随附**原推剪报摘录**——剥离 t.co 短链、折叠空白的原文片段，两行裁切（`-webkit-line-clamp:2`），`1px` 左引线（`#dededf`）+ 降调灰（`#656568`、`0.78rem`）标记"原文声音"，与解析的主角层级区分；再下一行是 quiet 标签登记行（`0.67rem`、`·` 分隔，纯文本不用徽章）。
...
- **原推剪报**样张贴片：细线框（`1px var(--line)`、`.75rem` 圆角）把原推"贴"在页面上——片内署名行（作者名 620 墨色 + @handle + 原推发布日期，`·` 分隔）、原文（t.co 短链就地替换为带下划线的展开链接）、引用原文（降调嵌套）与媒体都收进贴片；媒体只认 `12px` 圆角与 hover 缩放，不另加阴影。
