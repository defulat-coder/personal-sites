...
- GitHub、语雀、关于我、我的一天构成身份轨的链接行：quiet 文本 + 图标的同行链接，墨色 hover；均为外链或弹层，站内入口不进身份轨。「我的一天」使用原生对话框承载一支无声 16:9 个人叙事视频，不自动播放，保留原生控制条与封面；Esc、背板和关闭按钮均可退出，关闭时暂停播放。
...
- `/works` 是稳定策展而非每日流，刊名统一为「构建」：项目是能力、实验、决策、实践与里程碑的唯一根上下文，不建立脱离项目的全局实验池。网站只读取 Supabase `project_public_snapshots` 中审核通过的项目级公开快照；本机 Git、文档、Codex 证据、草稿和检查点不进入运行时读取面。
...
- `资料截至` 表示公开内容覆盖的最近证据时间，Supabase `published_at` 表示当前项目行的最近发布时间；采集与待审阶段不改变公开读取面，显式发布则按项目 upsert 后回读验证。重复发布同一 `revision` 仍会刷新 `published_at`，回读失败也不会自动回滚已完成的 upsert。
...
- 检索范围在组合器底部左侧（`DropdownMenu`）：触发器显示当前范围，点击向上展开单选列表（全部/每日动态/每日关注/开源关注）；不使用平铺的分段控件。
