...
- 简介下方是一张**双曲线成长图**：主线（工程 · 业务）从 2014 年低点出发持续缓坡爬升，代表传统 Java、服务运维与业务能力的积累；2023 年处是一个空心环节点，从这里分出第二条更陡的支线（AI 能力），弯向右上角，以实心墨点收尾并带轻微脉冲——"主干生长，然后开分支"。
- 加载时主线先随 `clip-path` 从左向右揭幕，支线延迟后在分支点开始描绘；主线下方面积以极浅墨色（约 5% 透明度）填充，年份节点随线到达逐个浮现，能力标注（工程 · 业务 / AI 能力）随所属线段完成后淡入。
- 只保留年份与领域词（如 `2014 Java·运维` → `2026 智能体平台`），不出现公司名、不加粗历史条目；当前阶段的标签是唯一使用墨色加粗的元素。
- SVG 使用 `preserveAspectRatio="none"` + `vector-effect="non-scaling-stroke"` 保证描边恒为 1.5px；揭幕动画用 `clip-path` 而非 dash 描边（非均匀缩放下 dash 间距会失真）。
- `prefers-reduced-motion` 下曲线、节点与标签全部静态立即可见。
