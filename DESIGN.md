...
- 刊头承载全部内容入口（每日动态/每日关注/开源关注/构建/问一问）：问一问虽是对话式动作，仍与阅读版块等权出现在版块序列末位；`/ask` 页的刊名是"问一问"，兄弟位与其余栏目页一致。
...
- 原始来源通栏（eyebrow 在上、细线链接列表）；**相邻剪报**收束阅读：页底双列登记行（上一则·较新收录 / 下一则·较早收录，quiet 标签 + stream headline 题名，第二条右对齐）；单侧存在时"下一则"仍落右列（`data-dir` + `:only-child`），窄屏单列复位；排序与列表完全一致。
