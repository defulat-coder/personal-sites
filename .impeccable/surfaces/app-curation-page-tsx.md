...
# Surface: 每日关注（/curation 列表 + /curation/[id] 详情）

- Scope: 汇集 X 与抖音公开关注条目的两个路由；组件 CurationStream 同时服务首页遗留 ?view=daily。
- Mode: Read——访客通过"他关注什么 + 他怎么判断"评估其工程判断力。
- Audience/job: 同行工程师与潜在合作方，快速扫读判断质量；陈远自己的日常回读入口。
- Proof/content: 判断题名 + 策展解析（主角）、来源摘录（X 原文或抖音转写/OCR 证据）、附件登记词、标签行；详情页为 sticky 样张贴片 + 深度解析对页。
