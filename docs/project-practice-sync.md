...
通用能力位于个人站：项目登记、适配器协议、增量状态机、提炼、审核、快照生成、发布与校验。各业务项目无需依赖个人站代码，也不直接写 Supabase。
...
- **公开修订**只在 Supabase 写入和回读校验都成功后推进。网站始终读取最近一次成功公开修订。
...
## 8. 私有存储与 Supabase 公开投影

原始证据、模型输入、待审草稿、检查点和失败队列只保存在本机敏感目录。Supabase 首版只接收审核后的公开快照。

建议使用一张 `project_public_snapshots` 表，每个项目一行：
...
| `snapshot jsonb not null` | 项目详情所需的已批准记录与公开证据 |

项目详情始终按项目整体读取，因此单行 JSONB 快照能提供原子发布，不需要通过多次 Data API 请求拼接半完成状态。列表查询只选择顶层列，详情查询才选择 `snapshot`。若未来确实需要跨项目实验检索，再从公开快照派生独立索引，不把索引反过来当内容真相来源。

安全边界：

- 表位于 Data API 可读取的 schema 时必须启用 RLS。
- `anon` 与 `authenticated` 只有 `SELECT` 已发布投影的权限；浏览器没有写权限。
- 发布脚本只在本机使用 service-role key，密钥不进入 `NEXT_PUBLIC_*`。
- 直接 upsert 单行即可完成原子发布，不新增公开数据库函数；若以后引入 RPC，必须显式收紧函数执行权限。
...
- `--engine codex-cli` 是显式备用提炼器；默认仍为 Pi / Kimi。
...
| Supabase 写入或回读失败 | 不推进公开修订；同一快照可幂等重发 |
...
5. 后续再接定时自动化、GitHub Issue/PR 适配器和“问一问”索引；这些不阻塞当前项目档案闭环。
...
- 未批准记录、原始会话、本机绝对路径、凭据模式和敏感正文无法从 Supabase 或生产页面读取。
- `/works/[slug]` 只渲染该项目记录；跨项目关联必须显式链接，不能复制所有权。
- 发布成功必须以 Supabase 回读的 `revision` 与记录数一致为准，不能只以命令退出码判断。
